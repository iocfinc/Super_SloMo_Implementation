...
def main():
    ...
    # Note: Adding drop_last argument to avoid an index out of range issue when creating the frames.
    videoFramesloader = torch.utils.data.DataLoader(videoFrames, batch_size=args.batch_size, shuffle=False, drop_last = True)
    videoFramesloader_1 = torch.utils.data.DataLoader(videoFrames, batch_size=args.batch_size, shuffle=False, drop_last = False)
    print('with drop {}, without drop {}'.format(len(videoFramesloader), len(videoFramesloader_1)))
    ...
    with torch.no_grad():
        for _, (frame0, frame1) in enumerate(tqdm(videoFramesloader), 0):
            ...
            for batchIndex in range(args.batch_size):
                (TP(frame0[batchIndex].detach())).resize(videoFrames.origDim, Image.LANCZOS).save(os.path.join(outputPath, str(frameCounter + args.sf * batchIndex) + ".jpg"))
            frameCounter += 1
            ...
            for intermediateIndex in range(1, args.sf):
                ...
                for batchIndex in range(args.batch_size):
                    # CHangeds Arbitrarily to LANCZOS was BILINEAR
                    (TP(Ft_p[batchIndex].cpu().detach())).resize(videoFrames.origDim, Image.LANCZOS).save(os.path.join(outputPath, str(frameCounter + args.sf * batchIndex) + ".jpg"))
                frameCounter += 1
