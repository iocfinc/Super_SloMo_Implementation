"""
eval.py will convert a video to a SuperSloMo processed version
"""
...
torch.set_grad_enabled(False) # We do not want any updates
...
if device != "cpu":
    ...
    trans_backward = transforms.Compose([transforms.Normalize(mean = neg_mean, std = std), trans_backward])
...
def interpolate_batch(frames, factor):
    frame0 = torch.stack(frames[:-1]) #first
    frame1 = torch.stack(frames[1:]) #remaining
    i0 = frame0.to(device)
    i1 = frame1.to(device)
    ...
    for i in range(1, factor):
        ...
        frame_buffer.append(ft_p) # add the interpolated frame to the frame buffer (stacking)
    return frame_buffer
...
def load_batch(video_in, batch_size, batch, w, h):
    ...
    # preprocess the video to something that our network can take in.
    for i in range(batch_size):
        ok, frame = video_in.read()
        if not ok:
            # Check first if video is ok
            break
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) # Rearrange the color.
...
def convert_video(source, dest, factor, batch_size = 10, output_format = 'mp4v', output_fps = 30):
    ...
    while True:
        ...
        if len (batch) == 1:
            break
            # The batch should have more than 1 frame. Otherwise it would just be an image.
        done += len(batch) -1
...
@click.option('--output', help='Path to output file save')
@click.option('--batch', default=2, help='Number of frames processed in one forward pass')
@click.option('--scale', default=4, help='Scale Factor of fps')
@click.option('--fps', default=30, help='FPS of the output video')
